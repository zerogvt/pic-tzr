...
public class PicTeaser {
	...
	public static void createScenario(ArrayList<String> imgs) {
		...
		for (int i=0; i<imgs.size(); i++) {
			...
			policy[i] = PicTeaser.FocusPolicy.values()[new Random().nextInt(PicTeaser.FocusPolicy.values().length)];
		}
	}
	
	
	
	public static void displayImage(String imgfile, Shell shell, Display display, Canvas canvas, int screenHeight, int screenWidth, PicTeaser.FocusPolicy focusPolicy ) {
		...
		if ( f.exists() ) {
			System.out.println("tick " + imgfile );
			...
			if (focusPolicy == PicTeaser.FocusPolicy.SCALE) {
				if ( h > screenHeight ) {
			...
			if (focusPolicy == PicTeaser.FocusPolicy.NO_SCALE) {
				nh=h;
				nw=w;
				if ( nh > screenHeight ) {
			...
			if (focusPolicy == PicTeaser.FocusPolicy.CRAZY_SCALE) {
				nh=h;
				nw=w;
	...
	public static void displayImage(ArrayList<String> imgs, Shell shell, Display display, Canvas canvas, int screenHeight, int screenWidth, PicTeaser.FocusPolicy focusPolicy ) {
		//=========================
		/*
		if (nextimg==0 || nextimg%3==0) {
			imgfile = imgs.get(getRandomMinMax(0,imgs.size()-1));
		}
		*/
		nextimg++;
		int	next = getRandomMinMax(0,imgs.size()-1);
		imgfile = imgs.get(next);
		Image dispImage = null;
		if (imgfile==null) return;
		File f = new File(imgfile);
		int nh=0,nw=0;
	...
	public static int getNextImg(int numFiles) {
	...
	public static void main (String [] args) {
		...
		display.timerExec(1000, new Runnable() {
			...
			String nextimgpath;
		    public void run() {
		    	//if (canvas.isDisposed () && rep!=0) return;
		    	PicTeaser.FocusPolicy random_policy = PicTeaser.FocusPolicy.values()[new Random().nextInt(PicTeaser.FocusPolicy.values().length)];
		    	//Teaser.FocusPolicy random_policy=Teaser.FocusPolicy.SCALE;
		    	...
		    	if (rep%3==1) {
		    		random_policy = PicTeaser.FocusPolicy.CRAZY_SCALE;
		    		//secstonext=1000;
		    	}
		    	if (rep%3==2) {
		    		random_policy = PicTeaser.FocusPolicy.NO_SCALE;
		    		//secstonext=2000;
		    	}
		    	if (rep%3==0) {
		    		random_policy = PicTeaser.FocusPolicy.SCALE;
		    		//secstonext=3000;
		    	}
